...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sqlCheck = "SELECT * FROM workers WHERE phoneno = '$phoneno'";
    ...
    if ($resultCheck->num_rows > 0) {
        ...
        echo "<script>
                Swal.fire({
                    icon: 'error',
                    title: 'Oops...',
                    text: 'This phone number is already used by another worker.',
                });
              </script>";
    } else {
        ...
        if ($conn->query($sqlUpdate)) {
             echo "<script>
                Swal.fire({
                    icon: 'success',
                    title: 'Success!',
                    text: 'Worker record updated successfully.',
        ...
        if ($conn->query($sqlInsert)) {
          echo "<script>
                Swal.fire({
                    icon: 'success',
                    title: 'Success!',
                    text: 'Worker record added successfully.',
                    showConfirmButton: false
                }).then(function() {
                    window.location.href = '" . $_SERVER['PHP_SELF'] . "'; // Redirect after closing the alert
                });
...
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id'])) {
    ...
    if ($conn->query($sqlDelete)) {
        ...
        Swal.fire({
            icon: 'success',
            title: 'Success!',
            text: 'Worker record deleted successfully.',
            showConfirmButton: false
        }).then(function() {
                        ...
                        <div class="card">
                            <div class="card-body">
                                <form class="form-sample" method="POST">
                                    ...
                                    <div class="sub">
                                        <button type="submit" class="btn-lg btn-primary btn-icon-text" id="submitButton">
...
$(document).ready(function () {
    $('.table').DataTable({
        ...
        columnDefs: [
            { targets: 0, className: 'text-center' },  // Sr. No centered
            { targets: 1, className: 'text-left' },    // Worker Name left
            { targets: 2, className: 'text-right' },   // Phone Number right
            { targets: 3, className: 'text-left' },    // Status left
            { targets: '_all', className: 'dt-head-center' } // Center all headers
        ],
        // Sort by the status column (index 3), placing 'active' first and 'inactive' last
        order: [[3, 'asc'], [0, 'asc']]  // First, sort by status (asc), then by Sr. No (asc)
    });
});
